...
class ViewController: UIViewController {
    ...
    private let delayBetweenInferencesMs: Double = 4000
...
extension ViewController: CameraFeedManagerDelegate {
    ...
    func didOutput(pixelBuffer: CVPixelBuffer) {
        ...
        DispatchQueue.main.async {
            ...
            guard let inferences = self.result?.inferences else {
                return
            }
            for item in inferences {
                if !item.label.isEmpty {
                    print("=============================>DataSpeech\(String(describing: item.label))")
                    self.textToSpeech(voiceOutdata: item.label)
                    break
                }
            }
        }
    ...
    // MARK: Text to Speech
    func textToSpeech(voiceOutdata: String ) {
        guard let url = Bundle.main.url(forResource: "\(voiceOutdata)", withExtension: "m4a") else { return }
